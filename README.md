...
# 创建 .env 并编辑配置（Docker 从 .env 加载所有配置）
cp .env.example .env
# 编辑 .env 填入 CHANNELS、ENABLED_PLUGINS 等
...
支持通过 **.env 文件** 或环境变量配置。推荐复制 `.env.example` 为 `.env` 后编辑：
...
| **AUTH_USERS** | 用户账号配置 | 无 | 格式：`user1:pass1,user2:pass2`（DB未启用时使用） |
| **AUTH_TOKEN_EXPIRY** | Token有效期（小时） | `24` | JWT Token的有效时长 |
| **AUTH_JWT_SECRET** | JWT签名密钥 | 自动生成 | 用于签名Token，建议手动设置 |
| **DB_ENABLED** | 启用MySQL用户体系 | `false` | 启用后自动开启认证，支持注册、bcrypt 密码 |
| **DB_DSN** | MySQL连接串 | 无 | 或使用 DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME 分项配置 |
| **CAPTCHA_ENABLED** | 启用图形验证码 | `false` | 登录/注册时需验证，防暴力破解 |

MySQL 用户体系详见 [用户体系配置指南](docs/用户体系配置.md)。所有 API 均返回统一格式 `{code, message, data}`。
...
- `GET /api/auth/captcha` - 获取图形验证码（CAPTCHA_ENABLED 时登录/注册前调用）
- `POST /api/auth/login` - 用户登录，获取Token
- `POST /api/auth/register` - 用户注册（需 DB_ENABLED=true）
- `POST /api/auth/verify` - 验证Token有效性
- `POST /api/auth/logout` - 退出登录（客户端删除Token）
- `GET /api/user/profile` - 获取当前用户资料（需认证）

**使用Token调用API：**

```bash
# 1. 登录获取Token（MySQL 用户需先注册）
curl -X POST http://localhost:8888/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"admin123"}'

# 响应：{"code":0,"message":"success","data":{"token":"eyJhbGc...","expires_at":1234567890,"username":"admin"}}
...
**说明**：启用 MySQL（DB_ENABLED=true）时，认证会自动开启，搜索等接口需携带 JWT。
...
当启用认证功能（`AUTH_ENABLED=true` 或 `DB_ENABLED=true`）时，除登录、注册、验证码、健康检测接口外的所有 API 接口都需要提供有效的 JWT Token。
...
| captcha_id | string | CAPTCHA_ENABLED 时 | 验证码 ID |
| captcha_answer | string | CAPTCHA_ENABLED 时 | 验证码答案 |
...
**是否需要认证**：取决于认证配置（`AUTH_ENABLED` 或 `DB_ENABLED=true` 时需携带 JWT）
...
**字段说明**（`total`、`results`、`merged_by_type` 均在 `data` 内）：
...
- `plugin_count`: 已启用的插件数量（仅插件启用时返回）
- `plugins`: 已启用的插件列表（仅插件启用时返回）
...
https://registry.docker-cn.com（官方中国镜像，可能不稳定）
...
方案四：改用更常见的 Alpine 版本
如果 3.19 拉取失败，可以把 Dockerfile 里的 alpine:3.19 改成 alpine:3.18 或 alpine:3.17，这些版本更容易命中缓存或镜像源。需要的话我可以帮你改 Dockerfile。
